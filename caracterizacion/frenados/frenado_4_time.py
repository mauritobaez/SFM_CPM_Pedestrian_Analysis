...
velocities_before_stopped_time = {}
for key in keys:
    velocities_before_stopped_time[key] = []
    for i in range(len(index_when_stopped[key])):
        if i == 0:
            continue
        curr_index = index_when_stopped[key][i]
        ...
        if curr_data_before_stopped:
            velocities_before_stopped_time[key].append(curr_data_before_stopped)
...
for key in keys:    
    ...
    fig.add_trace(go.Scatter(x=time[key], y=velocities[key], mode='lines', name=f'v {key}', line=dict(color='blue'), showlegend = False))
    ...
    for index, stopped_velocities in enumerate(velocities_before_stopped_time[key]):
        fig.add_shape(
            type="rect",
            x0=stopped_velocities[0][1],
            x1=stopped_velocities[-1][1],
    ...
    fig.add_trace(go.Scatter(
        x=[None], y=[None],
        mode='markers',
        marker=dict(size=10, color='#C55FA0', opacity=0.9),
        name='Both'
    ))
    ...
    fig.write_image(f"./imagenes2/{key}_all_time4metros_frenado.png", width=800, height=600)
    #fig.show()
