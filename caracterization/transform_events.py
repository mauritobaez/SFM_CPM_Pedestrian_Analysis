...
for key in keys:
    ...
    for i, event in enumerate(events):
        ...
        if WITH_NOTHING_TOO:
            fig.add_trace(go.Scatter(x=t, y=v_with_nothing, mode='lines', name='Raw Velocity', line=dict(color='blue'), opacity=0.6, line_width=2))
        fig.add_trace(go.Scatter(x=t, y=v, mode='lines', name='FFT Filtered Velocity 1.0', line=dict(color='red'), opacity=1, line_width=4))
        
        
        middle = get_middle(y if i == 2 or i == 5 else x, AMOUNT_ZEROES)
        add_vertical_line(fig, t[middle], color='blue', width=3, showlegend=True, legend='Middle point')
        ...
        if ACC:
            ...
            fig.add_trace(go.Scatter(x=ts, y=theoretical_v, mode='lines', name='Theoretical Velocity', line=dict(color='blue'), line_width=4))
            if f'event_{i+1}' in doubles:
                ...
                fig.add_trace(go.Scatter(x=t_first, y=theoretical_v_first, mode='lines', name='Theoretical Velocity (in 2)', line=dict(color='purple', dash='dash'), line_width=4))
                fig.add_trace(go.Scatter(x=t_second, y=theoretical_v_second, mode='lines', showlegend=False, line=dict(color='brown', dash='dash'), line_width=4))
        ...
        if DEC:
            ...
            add_vertical_line(fig, best_time, color='green', width=4, showlegend=True, legend='Start Deceleration')
            if DOUBLE_LINES:
                ...
                fig.add_trace(go.Scatter(x=first_time, y=first_line, mode='lines', name='Best Fit', line=dict(color='purple', dash='dash', width=4)))
                fig.add_trace(go.Scatter(x=second_time, y=second_line, mode='lines', showlegend=False,  line=dict(color='purple', dash='dash', width=4)))
            ...
            if DEC_EXP:
                ...
                fig.add_trace(go.Scatter(x=t_dec, y=theoretical_v_dec, mode='lines', name='Theoretical Deceleration', line=dict(color='blue'), line_width=3))
                ...
                if MODEL == 'SFM':
                    ...
                    fig.add_trace(go.Scatter(x=t_dec, y=theoretical_v_dec_follow, mode='lines', name='TD (Distance based)', line=dict(color='brown', dash='dot'), line_width=5))
                    ...
                    fig.add_trace(go.Scatter(x=t_dec, y=theoretical_v_dec_vm_fix, mode='lines', name='TD (Vm set)', line=dict(color='black', dash='dashdot'), line_width=5))
                    ...
                    fig.add_trace(go.Scatter(x=t_dec, y=theoretical_v_dec_both, mode='lines', name='TD (Both params fitted)', line=dict(color='magenta', dash='longdash'), line_width=5))
        ...
        fig.update_layout(
            ...
            showlegend=True,
            font=dict(size=30),
            xaxis=dict(title_font=dict(size=30), tickfont=dict(size=26), range=[0 if not DEC else t[middle], t[-AMOUNT_ZEROES-1]]),
            yaxis=dict(title_font=dict(size=30), tickfont=dict(size=26), range=[-0.3, 2]),
        )
