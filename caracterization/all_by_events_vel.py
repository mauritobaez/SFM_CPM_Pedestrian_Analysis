...
FILES_TO_USE = [7]  # Use all files from 01 to 14
folder_names = ['fft_with_30_zero'] #['NewPedestriansMovAvg_5PS_Ham']#  # Change this to the folder you want to use
...
for key in keys:
    ...
    for folder_name in folder_names:
        ...
        for i, event in enumerate(events):
            if f'{key}_{i+1:02}' not in figures:
            ...
            if folder_name == 'fft_with_30_zero':
                ...
                if middle is not None:
                    add_vertical_line(fig, time[middle] - time[stops[i][0] + end_prev_stop], color='blue', width=4, showlegend=True, legend="Middle", dash="dash")
                    
                add_vertical_line(fig, time[beg_next_stop] - time[end_prev_stop], color='orange', width=4, showlegend=True, legend="Start of Stop", dash="solid")
            
            fig.add_trace(go.Scatter(x=shifted_time, y=event, mode='markers', marker=dict(color='red' if folder_name == 'fft_with_30_zero' else 'blue', size=5), opacity=1, showlegend=False))
    ...
    for i, event in enumerate(events):
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                ...
                #range=[-1, 12]  # Set x axis range
            ),
            yaxis=dict(title_font=dict(size=26), tickfont=dict(size=24)),
